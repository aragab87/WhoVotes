...
# 4. code dummies to use in regression
  ...
  df_d <- df_d %>%
    mutate(green_voter = NA,
           green_voter = replace(green_voter, prtvede1 == "Alliance 90/The Greens (Bündnis 90/Die Grünen)", 1),
           green_voter = replace(green_voter, prtvede2 == "Alliance 90/The Greens (Bündnis 90/Die Grünen)", 1),
           green_voter = replace(green_voter, is.na(green_voter) == TRUE, 0),
    )
  ...
  df_d_r <- df_d %>%
    select(green_voter, is_male, income_above_median, above_35_yrs, attended_college, lives_village, religious, migration_background, married, dweight)
  
  # summary stats
  summary(df_d_r)
    
  # save data
  write.csv(df_d_r, file = "clean_data/18_germany_demographics_greens.csv",row.names=FALSE)
